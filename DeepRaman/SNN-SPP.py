...
class basemodel:
    def __init__(self):
        ...
        self.block1_conv1 = tf.keras.layers.Conv1D(64,7,activation = 'relu',padding = 'same',name = 'block1_conv1')
        self.block1_pool = tf.keras.layers.MaxPooling1D(2, name = 'block1_pool')

    def call(self, inputs):
        x = inputs
        x = self.block1_conv1(x)
        outputs = self.block1_pool(x)
        return outputs
...
def siamese(inputs):
    ...
    poolA = base_model.call(inputA)
    poolB = base_model.call(inputB)

    
    con = tf.keras.layers.concatenate([poolA,poolB],2)
    con = tf.expand_dims(con, -1)
    
    conv1 = tf.keras.layers.Conv2D(32, kernel_size=(7,7), strides=(2,2), padding='SAME',
                                   activation='relu',kernel_initializer='he_normal')(con)
